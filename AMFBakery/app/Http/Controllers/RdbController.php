...
class RdbController extends Controller
{
    ...
    {
        {
            $debugInfo = []; // Debugging information array
            ...
            $debugInfo['uploaded_file_path'] = $path;
            ...
            if (!file_exists($outputDir)) {
                ...
                $debugInfo['output_dir_created'] = $outputDir;
            } else {
                $debugInfo['output_dir'] = $outputDir;
            }
            ...
            $debugInfo['process_output'] = $process->getOutput();
            $debugInfo['process_error'] = $process->getErrorOutput();

            // Check for errors
            if (!$process->isSuccessful()) {
                $debugInfo['process_successful'] = false;
                throw new ProcessFailedException($process);
            }
            $debugInfo['process_successful'] = true;

            // Get the list of generated CSV files
            $csvFiles = array_diff(scandir($outputDir), ['.', '..']);
            $debugInfo['generated_csv_files'] = $csvFiles;
